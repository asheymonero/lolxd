...
void InProcessNode::init(const Callback& callback) {
  ...
  if (state != NOT_INITIALIZED) {
    ...
    // workGuard is already constructed in the header (keeps ioService.run() alive)
...
bool InProcessNode::doShutdown() {
  ...
  workGuard.reset();      // allow run() to exit once queued work is done
  ioService.stop();
  workerThread->join();
  ioService.restart();    // io_context uses restart() instead of reset()
  return true;
}
